...
def run_workflow_part1(company_id: str) -> Dict[str, Any]:
    ...
    async def run_workflow_part1_async():
        ...
        state_dict = {
            ...
            "human_approval": False,  # Will be set by HITL
            "final_dashboard": "",
...
def _execute_workflow_part2_logic(company_id: str, workflow_state: Dict[str, Any], approval_result: str = "Approve") -> Dict[str, Any]:
    """
...
with DAG(
    dag_id="orbit_agentic_dashboard_dag_simple",
    ...
    @task
    def auto_approve_all(**context) -> Dict[str, Any]:
        """Temporarily auto-approve all workflows (HITL disabled)"""
        ti = context['ti']
        
        # Get workflow result from hitl_check
        hitl_check = ti.xcom_pull(task_ids='check_if_hitl_needed', key='workflow_result')
        if not hitl_check:
            hitl_check = ti.xcom_pull(task_ids='check_if_hitl_needed')
        
        workflow_result = hitl_check.get('workflow_result', hitl_check) if isinstance(hitl_check, dict) else hitl_check
        company_id = workflow_result.get('company_id') if isinstance(workflow_result, dict) else None
        
        risk_detected = workflow_result.get('risk_detected', False) if isinstance(workflow_result, dict) else False
    ...
    auto_approve_all_results = auto_approve_all.expand(workflow_result=hitl_check_results)
    ...
    def finalize_workflow(**context) -> Dict[str, Any]:
        """Finalize workflow after auto-approval"""
        ti = context['ti']
        
        # Get approval result (should be auto-approved)
        approval_data = ti.xcom_pull(task_ids='auto_approve_all')
        if not approval_data:
            approval_data = ti.xcom_pull(task_ids='auto_approve_all', key='return_value')
        
        workflow_result = approval_data.get('workflow_result', {}) if isinstance(approval_data, dict) else {}
        company_id = workflow_result.get('company_id')
        
        if not company_id:
            raise ValueError(f"Could not get company_id from approval_data: {approval_data}")
        
        # Call the helper function directly
        return _execute_workflow_part2_logic(company_id, workflow_result, "Approve")
    
    finalize_results = finalize_workflow.expand(workflow_result=auto_approve_all_results)
